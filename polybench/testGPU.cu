...
__device__ void kernel_adi(int tsteps,int n,DATA_TYPE* X,DATA_TYPE* A,DATA_TYPE* B)
{
    int t, i1, i2;
...
__device__ void kernel_atax(int nx, int ny, DATA_TYPE* a, DATA_TYPE* x, DATA_TYPE* y,DATA_TYPE* tmp)
{
    int i, j;
    for (i = 0; i < ny; i++)
        y[i] = 0;
...
__device__ void correlation(DATA_TYPE* data, DATA_TYPE* mean, DATA_TYPE* stddev, DATA_TYPE* symmat, int M, int N, DATA_TYPE float_n)
{
	int i, j, j1, j2;
...
/* Center and reduce the column vectors. */
	for (i = 0; i < N; i++)
		...
		{
			data[i*N+j] -= mean[j];
			data[i*N+j] /= sqrtf(float_n) * stddev[j];
		}
...
/* Main computational kernel. The whole function will be timed,
including the call and return. */
__device__ void kernel_covariance(int m, int n,DATA_TYPE float_n,DATA_TYPE *data,DATA_TYPE *symmat,DATA_TYPE *mean)
{
    int i, j, j1, j2;
...
/* Main computational kernel. The whole function will be timed,
including the call and return. */
__device__ void kernel_durbin(int n,DATA_TYPE* y,DATA_TYPE* sum,DATA_TYPE* alpha,DATA_TYPE* beta,DATA_TYPE* r,DATA_TYPE* out)
    {
    int i, k;
...
__device__ void kernel_dynprog(int tsteps, int length,
	DATA_TYPE* c,
...
__device__ void kernel_jacobi_2d_imper(int tsteps, int n, DATA_TYPE* A,DATA_TYPE* B)
{
    int t, i, j;
    for (t = 0; t < tsteps; t++)
    {
...
/* Main computational kernel. The whole function will be timed,
including the call and return. */
__device__ void kernel_ludcmp(int n,DATA_TYPE* A,DATA_TYPE* b,DATA_TYPE* x,DATA_TYPE* y)
{
    int i, j, k;
...
__device__ void kernel_reg_detect(int niter, int maxgrid, int length,DATA_TYPE* sum_tang,DATA_TYPE* mean,DATA_TYPE* path,DATA_TYPE* diff,DATA_TYPE* sum_diff)
{
    int t, i, j, cnt;
    //   int _PB_NITER = niter;
    //  int _PB_LENGTH = length;
...
/* Main computational kernel. The whole function will be timed,
including the call and return. */
__device__ void kernel_syr2k(int ni, int nj,DATA_TYPE alpha, DATA_TYPE beta,DATA_TYPE* C,DATA_TYPE* A,DATA_TYPE* B)
{
    int i, j, k;
...
/* Main computational kernel. The whole function will be timed,
including the call and return. */
__device__ void kernel_floyd_warshall(int _PB_N, DATA_TYPE *path)
{
    int i, j, k;
...
__global__ void myKernel(inputData *data, outputData* outD)
{
...
void launch(inputData *dataCPU, outputData* outD,  char* fileName){
	...
	inputData *dataGPU, *l_data;
    l_data = (inputData*)malloc(sizeof(inputData)* NUMBEROFTEST);
    outputData* outGPU, *out_data;
    out_data = (outputData*)malloc(sizeof(outputData)* NUMBEROFTEST);
    DATA_TYPE* d_x;
    DATA_TYPE* d_a;
    DATA_TYPE* d_beta;
    DATA_TYPE* d_r;
    ...
    for(; i<NUMBEROFTEST;i++){
        int nx = dataCPU[i].nx;
        int ny = dataCPU[i].ny;
        char choice = dataCPU[i].choice;
	 l_data[i].choice = choice;
        switch(choice){
                case 0:
                    cudaMalloc((void**)&d_x, nx * nx * sizeof(DATA_TYPE));
                    cudaMemcpy(d_x,dataCPU[i].x, nx * nx * sizeof(DATA_TYPE), cudaMemcpyHostToDevice);

                    cudaMalloc((void**)&d_a, nx * nx * sizeof(DATA_TYPE));
                    cudaMemcpy(d_a,dataCPU[i].a, nx* nx *sizeof(DATA_TYPE), cudaMemcpyHostToDevice);

                    cudaMalloc((void**)&d_result, nx * nx * sizeof(DATA_TYPE));
                    cudaMemcpy(d_result,outD[i].result, nx * nx * sizeof(DATA_TYPE), cudaMemcpyHostToDevice);
                    l_data[i].nx = nx;
                    l_data[i].ny = ny;
                    l_data[i].a = d_a;
                    l_data[i].x = d_x;
                    out_data[i].result = d_result;
                    break;
                case 1:
                    cudaMalloc((void**)&d_x, ny * sizeof(DATA_TYPE));
                    cudaMemcpy(d_x,dataCPU[i].x,  ny * sizeof(DATA_TYPE), cudaMemcpyHostToDevice);
                    cudaMalloc((void**)&d_a, nx * ny * sizeof(DATA_TYPE));
                    cudaMemcpy(d_a,dataCPU[i].a, nx* ny *sizeof(DATA_TYPE), cudaMemcpyHostToDevice);
                    cudaMalloc((void**)&d_tmp, nx * sizeof(DATA_TYPE));
                    cudaMemcpy(d_tmp,dataCPU[i].tmp, nx*sizeof(DATA_TYPE), cudaMemcpyHostToDevice);
                    cudaMalloc((void**)&d_result,  ny * sizeof(DATA_TYPE));
                    cudaMemcpy(d_result,outD[i].result, ny *  sizeof(DATA_TYPE), cudaMemcpyHostToDevice);
		    ...
		    l_data[i].tmp = d_tmp;
                    ...
                    break;
	       case 2:
                    cudaMalloc((void**)&d_x, nx * sizeof(DATA_TYPE));
                    cudaMemcpy(d_x,dataCPU[i].x,  nx * sizeof(DATA_TYPE), cudaMemcpyHostToDevice);
                    cudaMalloc((void**)&d_a, nx * ny * sizeof(DATA_TYPE));
                    cudaMemcpy(d_a,dataCPU[i].a, nx* ny *sizeof(DATA_TYPE), cudaMemcpyHostToDevice);
		    cudaMalloc((void**)&d_tmp, nx * sizeof(DATA_TYPE));
                    cudaMemcpy(d_tmp,dataCPU[i].tmp, nx*sizeof(DATA_TYPE), cudaMemcpyHostToDevice);
        	    ...
        	    l_data[i].nx = nx;
                    ...
                    l_data[i].x = d_x;
                    l_data[i].tmp = d_tmp;
		    l_data[i].float_n = dataCPU[i].float_n;
		    out_data[i].result = d_result;
		    break;
		...
		case 3:
		    ...
		    l_data[i].float_n = dataCPU[i].float_n;
                ...
                case 4:
                    ...
                    cudaMalloc((void**)&d_beta, nx * sizeof(DATA_TYPE));
                    cudaMemcpy(d_beta,dataCPU[i].beta, nx *sizeof(DATA_TYPE), cudaMemcpyHostToDevice);

                    cudaMalloc((void**)&d_r, nx * sizeof(DATA_TYPE));
                    cudaMemcpy(d_r,dataCPU[i].r, nx *sizeof(DATA_TYPE), cudaMemcpyHostToDevice);

                    cudaMalloc((void**)&d_result,  nx * sizeof(DATA_TYPE));
                    cudaMemcpy(d_result,outD[i].result,  nx*  sizeof(DATA_TYPE), cudaMemcpyHostToDevice);
		    l_data[i].tmp = d_tmp;
                    l_data[i].nx = nx;
                    l_data[i].ny = ny;
                    l_data[i].a = d_a;
                    l_data[i].x = d_x;
                    l_data[i].beta = d_beta;
                    l_data[i].r = d_r;
                    out_data[i].result = d_result;
                    break;
	      case 5:
		    ...
		    l_data[i].r = d_r;
                ...
                case 8:
                    ...
                    cudaMalloc((void**)&d_tmp, nx * nx * ny * sizeof(DATA_TYPE));
                    cudaMemcpy(d_tmp,dataCPU[i].tmp, nx * nx * ny * sizeof(DATA_TYPE), cudaMemcpyHostToDevice);

                    cudaMalloc((void**)&d_r, nx * nx * ny * sizeof(DATA_TYPE));
                    cudaMemcpy(d_r,dataCPU[i].r, nx * nx * ny * sizeof(DATA_TYPE), cudaMemcpyHostToDevice);

                    cudaMalloc((void**)&d_result,  nx * nx * sizeof(DATA_TYPE));
                    cudaMemcpy(d_result,outD[i].result, nx * nx*  sizeof(DATA_TYPE), cudaMemcpyHostToDevice);
 
		    l_data[i].niter = dataCPU[i].niter;
                    l_data[i].nx = dataCPU[i].nx;
                    l_data[i].ny = dataCPU[i].ny;
                    l_data[i].a = d_a;
                    l_data[i].x = d_x;
                    l_data[i].tmp = d_tmp;
                    l_data[i].r = d_r;
               ...
               case 9:
                    ...
                    cudaMalloc((void**)&d_x, nx * ny * sizeof(DATA_TYPE));
                    cudaMemcpy(d_x,dataCPU[i].x, nx * ny *sizeof(DATA_TYPE), cudaMemcpyHostToDevice);
                    ...
                    l_data[i].x = d_x;
                    l_data[i].float_n = dataCPU[i].float_n;
                    l_data[i].n2 = dataCPU[i].n2;
		    out_data[i].result = d_result;
                    break;
		}
    	}

   	cudaMalloc ( (void**) &dataGPU, input_size);
    	cudaMemcpy(dataGPU, l_data, input_size, cudaMemcpyHostToDevice );

    	cudaMalloc ( (void**) &outGPU, output_size);
    	cudaMemcpy(outGPU, out_data, output_size, cudaMemcpyHostToDevice );

    	myKernel << <blocksPerGrid, threadsPerBlock >> >(dataGPU,outGPU); // excute on kernel

    	cudaMemcpy(out_data, outGPU, output_size, cudaMemcpyDeviceToHost);
    	i=0;
    	for(; i<NUMBEROFTEST;i++){
            switch(dataCPU[i].choice){
                case 0:
		cudaMemcpy(outD[i].result, out_data[i].result, dataCPU[i].nx * dataCPU[i].nx * sizeof(DATA_TYPE), cudaMemcpyDeviceToHost);
                    break;
                case 2:
		cudaMemcpy(outD[i].result, out_data[i].result, dataCPU[i].nx * dataCPU[i].nx * sizeof(DATA_TYPE), cudaMemcpyDeviceToHost);
                    break;
		...
		cudaMemcpy(outD[i].result, out_data[i].result, dataCPU[i].nx * dataCPU[i].nx * sizeof(DATA_TYPE), cudaMemcpyDeviceToHost);
                ...
                case 9:
                    cudaMemcpy(outD[i].result, out_data[i].result, dataCPU[i].nx * dataCPU[i].nx * sizeof(DATA_TYPE), cudaMemcpyDeviceToHost);
              	    break;
		 case 1:
                    cudaMemcpy(outD[i].result, out_data[i].result,  dataCPU[i].ny * sizeof(DATA_TYPE), cudaMemcpyDeviceToHost);
                    break;
                case 4:
		    cudaMemcpy(outD[i].result, out_data[i].result, dataCPU[i].nx * sizeof(DATA_TYPE), cudaMemcpyDeviceToHost);
	            break;
                case 7:
                    cudaMemcpy(outD[i].result, out_data[i].result, (dataCPU[i].nx+1) * sizeof(DATA_TYPE), cudaMemcpyDeviceToHost);
                    break;
            }
	}
	cudaFree(dataGPU);
